  ...
  function SafeLink({ href, children, className }) {
    ...
    const handleClick = (e) => {
      ...
      // Internal navigation we handle ourselves
      e.preventDefault();

      // Handle /legal and in-page anchors consistently
      if (href.startsWith("/legal")) {
        const [path, hashPart] = href.split("#");
        const hash = hashPart ? `#${hashPart}` : "";

        // If we’re already on /legal, just update the hash and scroll
        if (location.pathname === path) {
          if (hash) {
            // Update hash without full reload
            window.history.pushState({}, "", `${path}${hash}`);
          }
          // Scroll to target (your util handles this)
          const anchor = hashPart || "";
          goToLegalSection(navigate, location, undefined, anchor);
          return;
        }

        // Navigate to /legal, then scroll after paint
        navigate(`${path}${hash}`);
        setTimeout(() => {
          const anchor = hashPart || "";
          goToLegalSection(
    ...
    // External = real <a>; Internal = <a> but we intercept click
    if (isExternal) {
      return (
